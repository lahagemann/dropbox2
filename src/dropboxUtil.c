      ...
      while ((dir = readdir(d)) != NULL)
      {
            ...
            else
            {
                ...
                else
                {
                    ...
                    memcpy(&fi.last_modified,localtime(&(attrib.st_mtime)), sizeof(struct tm)); 
                    ...
                    if(index >= 0) // arquivo já existe na estrutura
                    {
                        ...
                        if(more_recent(fi.last_modified, f.last_modified)) 
                        {
...
//    memcpy(&T0, &diff_time(T1, T0), sizeof(struct tm));
    ret.tm_hour = ret.tm_hour/2;
    ret.tm_min = ret.tm_min/2;
    ret.tm_sec = ret.tm_sec/2;

    //struct tm Tc;
    Tc->tm_hour = Ts.tm_hour + ret.tm_hour;
    Tc->tm_min = Ts.tm_min + ret.tm_min;
    Tc->tm_sec = Ts.tm_sec + ret.tm_sec;
    mktime(Tc);
}
